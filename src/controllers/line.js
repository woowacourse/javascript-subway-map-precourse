...
export default {
  ...
  renderSelect() {
    const content = Station.getStations().reduce(
      (html, name) => html + `<option value="${name}">${name}</option>`,
      '',
    );
  ...
  renderList() {
    const content = Line.list().reduce(
      (html, line) =>
        html +
        `
      <tr>
  ...
  bindEvents() {
    this.query('#line-add-button').addEventListener(
      'click',
