...
export const DOMCtrl = {
  ...
  openStationManager() {
    const stationManager = `
      ...
      .map(
        station =>
          `<tr><td>${station}</td><td><button class="${DOMStrings.STATION_DELETE_BUTTON}" 
  ...
  openLineManager() {
    const lineManager = `
      <div id="${DOMStrings.LINE_MANAGER}"><br><span>${strings.LINE_NAME}</span><br>
      <input type="text" id="${DOMStrings.LINE_NAME_INPUT}" 
      placeholder="${strings.LINE_PLACEHOLDER}" /><br><br><span>${strings.LINE_START} 
      <select id="${DOMStrings.LINE_START_STATION_SELECTOR}">
      ${this.stations.map(station => `<option>${station}</option>`).join('')}</select></span>
      <br><span>${strings.LINE_END} <select id="${DOMStrings.LINE_END_STATION_SELECTOR}">
      ${this.stations.map(station => `<option>${station}</option>`).join('')}</select></span>
      <br><br><button id="${DOMStrings.LINE_ADD_BUTTON}">${strings.LINE_ADD}</button>
      <h1>${strings.LINE_LIST_TITLE}</h1><table id="${DOMStrings.LINE_LIST_TABLE}"><tr>
      <th><b>${strings.LINE_NAME}</b></th><th><b>${strings.LINE_START_STATION}</b></th>
      <th><b>${strings.LINE_END_STATION}</b></th><th><b>${strings.SETTING}</b></th></tr>
      ${this.lines
        .map(
          line =>
            `<tr><td>${line.lineName}</td><td>${line.start}</td>
          <td>${line.end}</td><td><button class="${DOMStrings.LINE_DELETE_BUTTON}" 
          data-${dataStrings.DATA_LINE}="${line.lineName}">${strings.DELETE}</button></td></tr>`
        )
        .join('')}</table></div>
    `;
    DOMCtrl.clearManagerContainer();
    ...
    const sectionContainer = `
      <div id="${DOMStrings.SECTION_CONTAINER}"><h2>${strings.SECTION_SELECT_TITLE}</h2>
        ${this.lines
          ...
          .join(' ')}
        <div id="${DOMStrings.SECTION_MANAGER}"></div></div>
    `;
    DOMCtrl.clearManagerContainer();
  ...
  openMapPrintManager() {
    const mapManager = `<div class="${DOMStrings.MAP_PRINT_MANAGER}">
      ${this.lines
        .map(
          line => `
  ...
  openSection(lineSelect, lineIndex) {
    const sectionManager = `
      <h2 data-${dataStrings.DATA_TARGET}="${lineSelect}">${lineSelect} ${strings.MANAGE}</h2><h3>
      ${strings.SECTION_ADD_TITLE}</h3><select id="${DOMStrings.SECTION_STATION_SELECTOR}">
      ${this.stations.map(station => `<option>${station}</option>`).join('')}</select>
      <input type="number" id="${DOMStrings.SECTION_ORDER_INPUT}" 
      placeholder="${strings.ORDER}">&nbsp;<button id="${DOMStrings.SECTION_ADD_BUTTON}">
      ${strings.ADD}</button>
      <br><br><br><table id="${DOMStrings.SECTION_LIST_TABLE}"><tr><th><b>${strings.ORDER}</b>
      </th><th><b>${strings.NAME}</b></th><th><b>${strings.SETTING}</b></th></tr>
      ${this.lines[lineIndex].stations
        .map(
          ...
          ${strings.SECTION_DELETE}</button></td></tr>`
        )
        .join('')}</table>
    `;
