...
- [X] 역 관리 기능
  ...
  - [X] 추가된 역 목록을 볼 수 있다.
  - [X] 역을 삭제할 수 있다.
    - [X] 노선에 추가된 역은 삭제할 수 없다.
- [X] 노선 관리 기능
  - [X] 노선 이름 입력 화면을 볼 수 있다.
  - [X] 상행, 하행 종점을 선택하는 화면을 볼 수 있다.
  - [X] 노선을 추가할 수 있다.
    - [X] 중복되는 노선은 추가할 수 없다.
    - [X] 상행 종점역과 하행 종점역이 같을 순 없다.
    - [X] 노선 이름은 공백일 수 없다.
    - [X] 상행 종점역과 하행 종점역이 같은 노선이 이미 존재하면 추가할 수 없다.
  - [X] 추가된 노선 목록을 볼 수 있다.
  - [X] 노선을 삭제할 수 있다.
- [X] 구간 추가 기능
  - [X] 구간을 수정할 노선을 선택하는 화면을 볼 수 있다.
  - [X] 선택한 구간을 클릭하면 구간을 추가할 수 있는 입력창을 볼 수 있다.
  - [X] 구간을 추가할 수 있다.
    - [X] 구간에 중복되는 역은 추가할 수 없다.
    - [X] 원하는 순서에 역을 추가할 수 있다.
    - [X] 구간 범위에 벗어나는 순서는 추가할 수 없다.
  - [X] 선택한 라인의 구간 목록을 볼 수 있다.
- [X] 구간 삭제 기능
  - [X] 해당 구간에 역이 2개 이하이면 삭제할 수 없다.
- [X] 노선도 출력 기능
  - [X] 노선의 상행 종점부터 하행 종점까지 연결순으로 볼 수 있다.
- [X] localStorage를 이용해 새로고침해도 최근 데이터를 볼 수 있다.
- [X] 재사용 가능한 Table 컴포넌트를 만들고 사용한다.