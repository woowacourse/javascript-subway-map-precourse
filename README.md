...
- [x] <del>상/하행 종점 순서가 **저장된 지하철 역 순서**와 반대인 경우 자동으로 상/하행 종점을 swap한다.</del>
- [x] 상/하행 종점은 같을 수 없다.
- [x] 초기 지하철 역이 1개 이하인 경우, 노선관리/구간관리/지하철노선도출력 버튼에 접근 할 수 없고 **역 관리** 화면만 접근 할 수 있다. (역 추가만 가능)
- [x] 노선에 포함된 역이 **2개 이하**이면 해당 노선에서 역을 삭제할 수 없다.
- [x] <del>지하철 노선 목록은 **노선 이름에 대해 오름차순**으로 정렬되어 노출된다.</del>
- [x] 노선이 없을 시, 구간관리/지하철 노선도 노출불가 처리
...
- [x] 입력한 구간 순서가 노선의 길이 범위 내에 존재하는지 확인하기
- [x] <del>노선의 종점이 같은 경우 확인하기.</del>
