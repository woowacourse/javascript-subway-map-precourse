...
- 지하철 노선도 출력 버튼을 클릭한 경우, `<div class="map"></div>` 태그를 만들고 해당 태그 내부에 노선도를 출력한다.
...
- 4가지 관리메뉴 버튼을 화면에 표시한다.

### 1. 역 관리 기능

- 역 등록 입력폼과 역 목록을 표시한다.
- 사용자가 `역 추가`버튼을 클릭한 경우, 입력된 역 이름을 검사한다.
  - 검사를 통과한 역을 목록에 추가한다.
  - [예외상황]
    - 지하철역 이름이 공백만으로 이루어진 경우
    - 지하철역 이름이 2글자 미만인 경우
    - 이미 등록된 지하철 이름인 경우
- 사용자가 `삭제`버튼을 클릭한 경우, 선택한 역을 검사한다.
  - 검사를 통과한 역을 목록에서 삭제한다.
  - [예외상황]
    - 노선에 등록된 역을 삭제하려는 경우

### 2. 노선 관리 기능

- 노선 등록 입력폼과 노선 목록을 표시한다.
- 사용자가 `노선 추가`버튼을 클릭한 경우, 입력한 노선 이름을 검사한다.
  - 검사를 통과한 노선을 목록에 추가한다.
  - [예외상황]
    - 노선 이름이 공백만으로 이루어진 경우
    - 상행 종점역과 하행 종점역이 같은 역일 경우
    - 이미 등록된 노선 이름인 경우
- 사용자가 `삭제`버튼을 클릭한 경우, 선택한 노선을 삭제한다.

### 3. 구간 관리 기능

- 구간 등록 입력폼과 노선 목록을 표시한다.
- 사용자가 `등록`버튼을 클릭한 경우, 입력한 순서와 구간을 검사한다.
  - 검사를 통과한 구간을 노선에 추가한다.
  - [예외상황]
    - 순서값으로 유효한 숫자가 입력되지 않은 경우
      - '', '--3', '++3', '1-1', '+-1', '-+1' 모두 이 경우에 해당
    - 순서값이 소수인 경우
    - 순서값이 양수가 아닌 경우
    - 순서값으로 숫자가 아닌 값을 입력하는 경우
    - 이미 해당 노선에 등록된 경우
- 사용자가 `노선에서 제거`버튼을 클릭한 경우, 선택한 구간을 검사한다.
  - 검사를 통과한 구간을 목록에서 삭제한다.
  - [예외상황]
