...
- 상단 버튼이 눌리면, 출력되던 결과물이 새로 나올 결과물과 같은 내용인지 확인하여 다르면 이전 결과물을 제거한다.
...
- 역 추가 버튼이 눌렸을 때, 역 이름이 중복되는지, 2글자 이상인지 확인한다.
- 노선 추가 버튼이 눌렸을 때, input의 값이 기존의 노선 이름들과 중복되는지 확인한다.
- 확인한 input 값이 문제가 있으면 경고창을 띄운다.
- 상행 종점과 하행 종점 select의 기본 값이 전체 역 중 가장 처음 등록된 역으로 설정되도록 한다.
- 구간 등록 부분에서 추가 될 구간의 순서가 0과 같거나 크면서 선택 된 노선의 구간들의 최대 순서 +1보다 같거나 작도록 한다.
- 구간 등록 부분에서 select의 기본 값이 전체 역 중 가장 처음 등록된 역으로 설정되도록 한다.
