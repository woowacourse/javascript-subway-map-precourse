...
- 테이블의 각 행들에 data로 인덱스를 설정해 준다.
- 역 추가 버튼이 눌렸을 때, 역 이름이 중복되는지, 2글자 이상인지, 공백이 있는지 확인한다.
- 노선 추가 버튼이 눌렸을 때, input의 값이 중복되거나 공백이 있는지 확인한다.
- 확인한 input 값이 문제가 있으면 경고창을 띄운다.
- 상행 종점과 하행 종점의 기본 역은 가장 처음 등록된 역으로 설정되도록 한다.
- 추가 될 구간의 순서가 0에서 현재 최대 순서 +1 사이가 되도록 한다.
- 추가 될 구간의 기본 역이 가장 처음 등록된 역으로 설정되도록 한다.
