...
## 기능 목록 작성
- [ ] 초기 화면은 **역 관리** 화면 노출
- [ ] "1. 역 관리" 버튼 클릭 시 **역 이름 입력란, 지하철 역 목록** 영역이 노출되어야 한다.
- [ ] "2. 노선 관리" 버튼 클릭 시 **노선 이름 입력란, 상/하행 종점, 지하철 노선 목록** 영역이 노출되어야 한다.
- [ ] "3. 구간 관리" 버튼 클릭 시 **수정할 노선 버튼** 영역이 노출되어야 한다.
- [ ] "4. 지하철 노선도 출력" 버튼 클릭 시 **모든 지하철 노선** 정보 영역이 노선별로 노출되어야 한다.
- [ ] "3. 구간 관리"의 수정한 노선 버튼을 클릭하면 해당 구간에 대한 **구간 등록 입력란/정보** 영역이 노출되어야 한다.
- [ ] 지하철 역 이름이 **2글자** 이상인지 체크해야 한다.
- [ ] 역/노선 추가를 하면 지하철 역/노선 목록 최하단에 추가된다.
- [ ] 자하철 역/노선 목록의 **삭제**버튼을 클릭 시, 얼럿 노출 확인 후 해당 요소가 제거된다.
- [ ] 노선에 등록된 역은 **삭제할 수 없다**.
- [ ] 노선관리 상/하행 종점의 default 값은 지하철 노선 목록의 **1행의 상행 종점역**으로 한다.
- [ ] 상/하행 종점 목록은 **저장된 지하철 역 순서**대로 노출된다.
...
- [ ] 모든 입력란에 대해 **빈칸 or 공백입력**인 경우를 체크한다.
- [ ] 역/노선 추가시, 역/노선 이름이 중복인지 체크한다.
- [ ] 구간 등록시, 순서에는 숫자만 입력가능해야 한다.
