...
- 상단의 4개의 버튼 중 아무거나 눌리면, 버튼 클릭 전에 아래에 출력되던 결과물이 현재 버튼이 눌림으로써 예상된 결과값과 같은 속성인지(역 관리인지, 노선 관리인지 같은) 확인하여 다른 속성일 경우, 그 전의 결과물을 제거한 뒤, 새로운 결과물을 출력하고 같은 속성일 경우 아무 일도 일어나지 않도록 해야 한다.
- 데이터에 변화가 생기면 즉각적으로 local storage값과 렌더링 결과를 업데이트한다.
- 데이터를 추가하면 데이터는 항상 끝 부분에 추가되도록 한다.

### 지하철 역 관련 기능

- 역 추가 버튼이 눌렸을 때, 사용자가 입력한 역 이름이 중복되는지, 2글자 이상인지, 공백이 있는지 확인하여 부적절한 경우 경고창을 띄워야 한다.
- 역 목록의 각 행들에 data로 인덱스를 설정해 준다.

### 지하철 노선 관련 기능

- 노선 추가 버튼이 눌렸을 때, input의 값이 중복되거나 공백이 있는지 확인하여 부적절한 경우 경고창을 띄워야 한다.
- 상행 종점과 하행 종점의 기본 역은 가장 처음 등록된 역으로 설정되도록 한다.
- 역 목록의 각 행들에 data로 인덱스를 설정해 준다.

### 지하철 구간 추가 기능

- 구간의 순서가 0에서 현재 최대 순서 +1 사이가 되도록 한다.

### 지하철 구간 삭제 기능

- 한 노선에 역이 2개일 때는 노선에서 역을 지우지 못하도록 한다.
- 역 목록의 각 행들에 data로 인덱스를 설정해 준다.
