...
- 역 관리
  - `localStorage` 및 `data`속성을 반영한다.
  - [x] `역 추가` 버튼을 눌러 역을 추가한다.
  ...
  - `삭제` 버튼을 눌러 역을 삭제한다.
    ...
    - 지하철 역 목록에 반영한다.
- 노선 관리
  - `노선 추가` 버튼을 눌러 노선을 추가한다.
    - 상행 종점과 하행 종점 목록을 역 목록에서 가져온다.
    - 입력한 노선 이름이 중복인지 검증해야 한다.
  ...
  - `삭제` 버튼을 눌러 노선을 삭제한다.
    ...
    - 노선 목록에 반영한다.
- 구간 관리
  - 구간을 수정할 노선의 목록을 버튼으로 출력한다.
  - 노선을 선택하면 관련 화면을 출력한다.
    - 제목은 `(선택된 노선) 관리`를 출력한다.
  - `등록` 버튼을 눌러 구간을 추가한다.
    - 입력한 역의 이름과 순서를 검증해야 한다.
    - 입력한 역의 이름이 중복인지 검증해야 한다.
    - 역과 역 사이(구간)인지 검증해야 한다.
    - 구간 목록에 반영한다.
  - `노선에서 제거` 버튼을 눌러 구간을 삭제한다.
    - 노선에 등록된 역이 두 개 이하인지 검증해야 한다. (두 개 이하인 경우에는 삭제 불가)
    - `alert`를 통해 다시 물어본다.
    - 구간 목록에 반영한다.
- 지하철 노선도 관리
  - 각 노선의 역들을 출력한다.
