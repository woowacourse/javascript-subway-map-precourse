...
지하철 관리 시스템입니다. 역과 노선을 추가(Create), 조회(Read), 삭제(Delete)할 수 있고, 원하는 노선에 역을 추가/노선에서 역을 제거할 수 있습니다. 지하철 노선도 출력 버튼을 누르면 노선 별로 등록된 역을 조회할 수도 조회할 수 있습니다. 일반적인 역이나 노선 이름의 규칙에서 벗어난 입력은 받지 않습니다🚨
