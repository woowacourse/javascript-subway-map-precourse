...
### 1. 역 관리
- 역 이름이 올바르지 않다면 경고 메시지와 함께 참고할 수 있는 사이트 링크를 부여한다.
- 삭제 버튼을 눌렀을 때 기존 노선에 존재하는 역이라면 경고 메시지와 함께 현재 역이 포함된 노선을 제시한다.
### 2. 노선 관리
- 노선의 이름은 미리 선언해두어 입력 값과 비교 후 노선을 추가하도록 한다.
- 노선 이름이 올바르지 않다면 경고 메시지와 함께 참고할 수 있는 사이트 링크를 부여한다.
- 노선의 종점은 상, 하행 통틀어 중복될 수 없다.
  - 중복 값 입력 시, 경고 메시지와 함께 중복 처리된 값을 제시한다.
### 3. 구간 관리
